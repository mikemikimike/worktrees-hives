...
- Command capability classification consumes wrapper-option operands, rejects wrapper environment assignments, ambiguous clusters, and executable read-only Git options, covers test-package `.__main__` aliases, and fails closed on unsupported wrapper escapes; shell executors do not collapse nested test or experiment requirements.
