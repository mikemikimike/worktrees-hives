...
- Detect `git` / `git.exe`, parse supported global options, and inspect the subcommand. Unchecked runtime configuration (`-c` / `--config-env`) and forced pagination (`-p` / `--paginate`) require the full conservative shell capability set because they may select an external process.
- Git classification is deny-by-default: only `_GIT_READ_ONLY_SUBCOMMANDS` and the explicitly inspected read-only forms require no capability. Options that write output map to `modify_code`. Read-oriented options that launch configured processes, such as `grep -O` / `--open-files-in-pager`, `cat-file --filters` / `--textconv`, and `diff` / `log` / `show` with `--ext-diff` or `--textconv`, require the full conservative shell capability set so nested test or experiment execution is not hidden. Every other subcommand maps to `modify_code`, including `add`, `am`, `apply`, `checkout`, `cherry-pick`, `clean`, `commit`, `merge`, `mv`, `pull`, `rebase`, `reset`, `restore`, `revert`, `rm`, `stash`, and `switch`. Basename `patch` also maps to `modify_code`.
- Supported command wrappers are parsed conservatively; environment assignments, ambiguous short-option clusters, and unsupported GNU `env -S` escapes fail closed with the full conservative shell capability set.
- `execute_tests`: basename `pytest`; or an unversioned/versioned console or windowed Python interpreter with `-m pytest`, `-m pytest.__main__`, `-m unittest`, or `-m unittest.__main__`; or `cargo test` (after cargo global options, treat `test` as the cargo subcommand).
...
- Any other recognized Python interpreter route, including `-c`, stdin, scripts, unrelated `-m` modules, and interactive `-i` module launches, requires the full conservative shell capability set.
