...
- Git classification is deny-by-default: only `_GIT_READ_ONLY_SUBCOMMANDS` and the explicitly inspected read-only forms require no capability. Options that write output map to `modify_code`. Read-oriented options that launch configured processes, such as `grep -O` / `--open-files-in-pager` and `cat-file --filters` / `--textconv`, require the full conservative shell capability set so nested test or experiment execution is not hidden. Every other subcommand maps to `modify_code`, including `add`, `am`, `apply`, `checkout`, `cherry-pick`, `clean`, `commit`, `merge`, `mv`, `pull`, `rebase`, `reset`, `restore`, `revert`, `rm`, `stash`, and `switch`. Basename `patch` also maps to `modify_code`.
...
- `launch_experiments`: argv `worktrees-hives lab …`, `wh-orch lab …`, an unversioned/versioned console or windowed Python interpreter running `-m worktrees_hives.cli lab …`, or first token `lab`.
- Any other recognized Python interpreter route, including `-c`, stdin, scripts, and unrelated `-m` modules, requires the full conservative shell capability set.
