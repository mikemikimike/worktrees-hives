...
- Detect `git` / `git.exe`, parse supported global options, and inspect the exact case-sensitive subcommand spelling so differently cased shell aliases cannot masquerade as built-ins. Unchecked runtime configuration (`-c` / `--config-env`) and a final forced-pagination setting (`-p` / `--paginate`) require the full conservative shell capability set because they may select an external process. The only no-capability runtime overrides are explicit false values for `core.fsmonitor` and `log.showSignature`. Read-only Git forms require `core.fsmonitor=false` and final pager suppression (`-P` / `--no-pager`) so ambient pagers and fsmonitor hooks cannot launch configured commands. Object-materializing reads also require an effective exact `env GIT_NO_LAZY_FETCH=1` assignment so a partial clone cannot invoke a promisor transport. A no-capability `status` additionally requires exact global `--no-optional-locks`, preventing an optional index refresh from invoking `post-index-change`. Pretty-rendering history forms additionally require `log.showSignature=false`.
- Git classification is deny-by-default: only `_GIT_READ_ONLY_SUBCOMMANDS` and the explicitly inspected read-only forms under the neutralized process configuration require no capability. Legacy `git config` actions are parsed only before the first positional operand, with `get` / `list` subcommands recognized explicitly, so values such as `--get` cannot convert a write into a classified read. Read-oriented forms that can launch configured processes require the full conservative shell capability set: partial-clone lazy fetches; implicit textconv for `diff`, `show`, and patch-rendering `log` / `whatchanged` (including clustered `-p` / `-u` flags); explicit pager/filter/textconv/ext-diff options (including `whatchanged --ext-diff`); `help` viewers; querying `remote show` or `ls-remote`; direct, configured-pretty, peeled, sort-key, ref-format, or rendering signature verification; `--alternate-refs` on any revision consumer; credential helpers; difftool commands; and archive formatters. Safe callers can explicitly disable implicit diff helpers with `--no-ext-diff` / `--no-textconv`, use the non-querying `remote show -n` / `ls-remote --get-url` forms, and select a built-in or inline `format:` / `tformat:` pretty format for default-pretty history commands. Every mutating or otherwise unclassified Git subcommand requires the full conservative shell capability set, including `modify_code`, because filters, hooks, transports, and shell aliases can execute beneath commands such as `add`, `checkout`, `commit`, `merge`, `pull`, and `push`. Basename `patch` remains mapped only to `modify_code`.
- Supported command wrappers are parsed conservatively; environment assignments other than the exact `GIT_NO_LAZY_FETCH=1` Git neutralizer, ambiguous short-option clusters, unsupported GNU `env -S` escapes, and BSD/macOS `env -P` command-lookup overrides fail to the full conservative capability set rather than hiding or weakening the nested command's requirements. Nested `env` wrappers preserve, clear, unset, or restore the lazy-fetch assignment in execution order; `-i`, `--ignore-environment`, bare `-`, and `-u` / `--unset GIT_NO_LAZY_FETCH` invalidate an outer assignment unless a later exact assignment restores it. The effective lazy-fetch assignment also fails closed when the eventual executable is not Git.
...
- `env GIT_NO_LAZY_FETCH=1 git -P --no-optional-locks -c core.fsmonitor=false status` requires no extra capability. The equivalent safe `log` form also disables ambient signature verification and selects an explicit built-in or inline pretty format, for example `env GIT_NO_LAZY_FETCH=1 git -P -c core.fsmonitor=false -c log.showSignature=false log --pretty=format:%H`. Without those neutralizers, even read forms conservatively account for configured process launchers. Unclassified non-Git executables impose no extra role capability: this mapping is deliberately not a universal command allowlist, as specified by the design.
