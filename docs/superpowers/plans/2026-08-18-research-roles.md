...
- Git classification is deny-by-default: only `_GIT_READ_ONLY_SUBCOMMANDS` and the explicitly inspected read-only forms require no capability. Every other subcommand maps to `modify_code`, including `add`, `am`, `apply`, `checkout`, `cherry-pick`, `clean`, `commit`, `merge`, `mv`, `pull`, `rebase`, `reset`, `restore`, `revert`, `rm`, `stash`, and `switch`. Basename `patch` also maps to `modify_code`.
- `execute_tests`: basename `pytest`; or an unversioned/versioned Python interpreter with `-m pytest` / `-m unittest`; or `cargo test` (after cargo global options, treat `test` as the cargo subcommand).
