...
- Detect `git` / `git.exe` then skip global options (`-C`, `-c`, `--git-dir`, `--work-tree`) the same way `lab_run` does, then inspect the subcommand.
- `modify_code` git subcommands: `add`, `commit`, `apply`, `rebase`, `cherry-pick`, `reset`, `restore`, `mv`, `rm`, `checkout`, `switch`. Also basename `patch`.
- `execute_tests`: basename `pytest`; or `python`/`python3` with `-m pytest` / `-m unittest`; or `cargo test` (after cargo global options, treat `test` as the cargo subcommand).
- `launch_experiments`: argv `worktrees-hives lab …`, `wh-orch lab …`, or first token `lab`.
